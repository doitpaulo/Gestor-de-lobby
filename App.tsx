...
const SprintsView = ({ tasks, sprints, setSprints, devs, user, onEditTask }: any) => {
                                        ...
                                        {selectedSprint.tasks
                                            ...
                                            return (
                                                <tr key={st.taskId} className="border-b border-slate-800 hover:bg-slate-800/30">
                                                    <td className="p-3">
                                                        <div 
                                                            className="flex flex-col cursor-pointer hover:bg-indigo-500/10 p-1 rounded transition-colors"
...
const DashboardView = ({ tasks, devs }: { tasks: Task[], devs: Developer[] }) => {
  ...
  const hoursByProjectData = useMemo(() => {
    return tasks
      .filter(t => {
        ...
        return t.subTasks && t.subTasks.length > 0 && matchesType;
      })
      .map(t => {
        const filteredSubTasks = (t.subTasks || []).filter(st => filterDev.length === 0 || filterDev.includes(st.assignee || ''));
        const est = filteredSubTasks.reduce((acc, st) => acc + (st.estimatedHours || 0), 0);
        const act = filteredSubTasks.reduce((acc, st) => acc + (st.actualHours || 0), 0);
        return { name: t.summary, estimated: est, actual: act, totalSubTasks: t.subTasks?.length || 0 };
      })
  ...
  const hoursByDevData = useMemo(() => {
    const devHours: Record<string, { estimated: number, actual: number }> = {};
    tasks.forEach(t => {
      const matchesType = filterType.length === 0 || filterType.includes(t.type);
      if (!matchesType) return;

      (t.subTasks || []).forEach(st => {
        if (st.assignee && (filterDev.length === 0 || filterDev.includes(st.assignee))) {
          if (!devHours[st.assignee]) devHours[st.assignee] = { estimated: 0, actual: 0 };
          devHours[st.assignee].estimated += (st.estimatedHours || 0);
          devHours[st.assignee].actual += (st.actualHours || 0);
        }
      });
    });
    return Object.entries(devHours)
      .map(([name, hours]) => ({ name, ...hours }))
      .sort((a, b) => b.estimated - a.estimated);
  }, [tasks, filterDev, filterType]);

  const estVsActData = useMemo(() => {
    let totalEst = 0;
    let totalAct = 0;
    tasks.forEach(t => {
      const matchesType = filterType.length === 0 || filterType.includes(t.type);
      ...
      if (t.subTasks && t.subTasks.length > 0) {
        t.subTasks.forEach(st => {
          if (filterDev.length === 0 || filterDev.includes(st.assignee || '')) {
            totalEst += (st.estimatedHours || 0);
            totalAct += (st.actualHours || 0);
          }
        });
      } else if (matchesDev) {
        totalEst += parseDuration(t.estimatedTime);
        totalAct += parseDuration(t.actualTime);
      }
    });
    return [
      { name: 'Estimado', value: totalEst },
  ...
  const renderWidget = (widget: Widget) => {
      ...
      const renderChartContent = () => {
          ...
          if (widget.type === 'hoursByProject') {
              return (
                  <BarChart data={hoursByProjectData} layout="vertical" margin={{ left: 20, right: 30 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#334155" horizontal={false} />
                      <XAxis type="number" stroke="#94a3b8" />
                      <YAxis type="category" dataKey="name" stroke="#94a3b8" width={150} tick={{fontSize: 10}} />
                      <Tooltip contentStyle={{ backgroundColor: '#1e293b', borderColor: '#475569' }} />
                      <Legend />
                      <Bar dataKey="estimated" name="Estimado (h)" fill="#6366f1" radius={[0, 4, 4, 0]} />
                      <Bar dataKey="actual" name="Realizado (h)" fill="#10b981" radius={[0, 4, 4, 0]} />
                  </BarChart>
              );
          }
          if (widget.type === 'hoursByDev') {
              return (
                  <BarChart data={hoursByDevData}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#334155" vertical={false} />
                      <XAxis dataKey="name" stroke="#94a3b8" tick={{fontSize: 10}} />
                      <YAxis stroke="#94a3b8" />
                      <Tooltip contentStyle={{ backgroundColor: '#1e293b', borderColor: '#475569' }} />
                      <Legend />
                      <Bar dataKey="estimated" name="Estimado (h)" fill="#6366f1" radius={[4, 4, 0, 0]} />
                      <Bar dataKey="actual" name="Realizado (h)" fill="#10b981" radius={[4, 4, 0, 0]} />
                  </BarChart>
              );
          }
          if (widget.type === 'estVsAct') {
              return (
                  <PieChart>
                      <Pie data={estVsActData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label>
                          {estVsActData.map((entry, index) => <Cell key={index} fill={index === 0 ? '#6366f1' : '#10b981'} />)}
                      </Pie>
                      <Tooltip contentStyle={{ backgroundColor: '#1e293b', borderColor: '#475569' }} />
                      <Legend />
                  </PieChart>
...
const TaskModal = ({ task, developers, allTasks, onClose, onSave, onDelete, workflowConfig }: any) => {
    ...
    const handleAddSubTask = () => {
        const newSubTask: SubTask = {
            ...
            actualHours: 0,
            status: 'Pendente',
            phase: formData.projectData?.currentPhaseId || '1'
        };
    ...
    const handleUpdateSubTask = (id: string, field: keyof SubTask, value: any) => {
        setFormData(prev => {
            const updatedSubTasks = (prev.subTasks || []).map(st => st.id === id ? { ...st, [field]: value } : st);
            
            // Auto-calculate hours
            const totalEst = updatedSubTasks.reduce((acc, st) => acc + (st.estimatedHours || 0), 0);
            const totalAct = updatedSubTasks.reduce((acc, st) => acc + (st.actualHours || 0), 0);
            
            return { 
                ...prev, 
                subTasks: updatedSubTasks,
                estimatedTime: `${totalEst}h`,
                actualTime: `${totalAct}h`
            };
        });
    };
    ...
    const handleDeleteSubTask = (id: string) => {
        setFormData(prev => {
            ...
            return { 
                ...prev, 
                subTasks: updatedSubTasks,
                estimatedTime: updatedSubTasks.length > 0 ? `${totalEst}h` : prev.estimatedTime,
                actualTime: updatedSubTasks.length > 0 ? `${totalAct}h` : prev.actualTime
            };
        });
    };
    ...
    <div className="bg-indigo-900/10 border border-indigo-500/30 p-4 rounded-lg space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
            <div>
                ...
                <select className="w-full bg-slate-900 border border-slate-600 rounded p-2 text-white outline-none focus:ring-2 focus:ring-indigo-500" value={formData.projectData?.currentPhaseId} onChange={(e) => { handleProjectDataChange('currentPhaseId', e.target.value); const newPhase = workflowConfig.find((w:any) => w.id === e.target.value); if (newPhase) handleProjectDataChange('phaseStatus', newPhase.statuses[0]); }}>
                    {workflowConfig.map((p: WorkflowPhase) => (<option key={p.id} value={p.id}>{p.name}</option>))}
                </select>
            </div>
            <div>
                <label className="block text-xs text-slate-400 mb-1 font-medium uppercase tracking-wider">Status da Fase</label>
                <select className="w-full bg-slate-900 border border-slate-600 rounded p-2 text-white outline-none focus:ring-2 focus:ring-indigo-500" value={formData.projectData?.phaseStatus} onChange={(e) => handleProjectDataChange('phaseStatus', e.target.value)}>
                    {currentPhase.statuses.map((s: string) => (<option key={s} value={s}>{s}</option>))}
                </select>
            </div>
        </div>
        ...
        <div className="pt-2 border-t border-indigo-500/20">
            <div className="flex justify-between items-center mb-3">
                <label className="block text-xs text-slate-400 font-medium uppercase tracking-wider">Subtarefas / Etapas de Desenvolvimento</label>
                <Button onClick={handleAddSubTask} variant="success" className="text-[10px] py-1 px-2"><IconPlus className="w-3 h-3" /> Adicionar Etapa</Button>
            </div>
            <div className="space-y-3">
                {(formData.subTasks || []).map(st => (
                    <div key={st.id} className="bg-slate-900 p-3 rounded-lg border border-slate-700 flex flex-col gap-3 hover:border-indigo-500/50 transition-colors">
                        <div className="flex gap-2 items-start">
                            <input 
                                value={st.title} 
                                onChange={e => handleUpdateSubTask(st.id, 'title', e.target.value)} 
                                placeholder="Nome da etapa (ex: Assessment, AS-IS, TO-BE)..." 
                                className="flex-1 bg-slate-800 border border-slate-600 rounded p-2 text-xs text-white outline-none focus:border-indigo-500"
                            />
                            <button onClick={() => handleDeleteSubTask(st.id)} className="text-rose-500 hover:text-rose-400 p-2 bg-slate-800 rounded border border-slate-600 hover:border-rose-500 transition-colors">✕</button>
                        </div>
                        <div className="grid grid-cols-4 gap-3">
                            <div>
                                <label className="text-[10px] text-slate-500 mb-1 block">Responsável</label>
                                <select 
                                    value={st.assignee || ''} 
                                    onChange={e => handleUpdateSubTask(st.id, 'assignee', e.target.value)}
                                    className="w-full bg-slate-800 border border-slate-600 rounded p-1.5 text-xs text-white outline-none focus:border-indigo-500"
                                >
                            ...
                            <div>
                                <label className="text-[10px] text-slate-500 mb-1 block">Est. (h)</label>
                                <input 
                                    type="number" 
                                    value={st.estimatedHours} 
                                    onChange={e => handleUpdateSubTask(st.id, 'estimatedHours', parseFloat(e.target.value) || 0)}
                                    className="w-full bg-slate-800 border border-slate-600 rounded p-1.5 text-xs text-white outline-none focus:border-indigo-500"
                                />
                            </div>
                            <div>
                                <label className="text-[10px] text-slate-500 mb-1 block">Real (h)</label>
                                <input 
                                    type="number" 
                                    value={st.actualHours} 
                                    onChange={e => handleUpdateSubTask(st.id, 'actualHours', parseFloat(e.target.value) || 0)}
                                    className="w-full bg-slate-800 border border-slate-600 rounded p-1.5 text-xs text-white outline-none focus:border-indigo-500"
                                />
                            </div>
                            <div>
                ...
                {(!formData.subTasks || formData.subTasks.length === 0) && (
                    <div className="text-xs text-slate-500 text-center py-4 bg-slate-900/50 rounded border border-slate-700 border-dashed">
                        Nenhuma etapa de desenvolvimento cadastrada. Clique em "Adicionar Etapa" para começar.
                    </div>
                )}
            </div>
        </div>
